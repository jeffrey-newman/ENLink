...
#include <stdio.h>
#include <string.h>
//#include <malloc.h> //JN Modified
#include <stdlib.h>  //JN Modified
#include <math.h>
#include "toolkit.h"
#include "hash.h"
#include "text.h"
...
extern int    Ntokens,           /* Number of tokens in input line    */
              Ntitle;            /* Number of title lines             */

static int  InpNcontrols;

static int  getpumpcurve(float, char **, int);  /* Constructs a pump curve   */
...
/*** Updated 6/24/02 ***/
    if (n >= 3)
    {
        ...
        Node[Njuncs].Ddefault = 1;
    }
    else Node[Njuncs].Ddefault = 0;
...
/* so retrieve pump curve parameters */
    if (getfloat (Tok[3], &y))
    {
        return getpumpcurve (y, Tok, n);          /* Get pump curve params */
    }
...
*/
{
    ...
    InpPattern[p->i].Length += n;         /* Save # multipliers for pattern */
...
/* Fill in fields of control data structure */
    InpNcontrols++;
    if (InpNcontrols > MaxControls) return(200);
    InpControl[InpNcontrols].Link     = k;
    InpControl[InpNcontrols].Node     = i;
    InpControl[InpNcontrols].Type     = type;
    InpControl[InpNcontrols].Status   = status;
    InpControl[InpNcontrols].Setting  = setting;
    InpControl[InpNcontrols].Time     = (long)(3600.0*time);
    if (type == TIMEOFDAY)
        InpControl[InpNcontrols].Time %= SECperDAY;
    InpControl[InpNcontrols].Grade    = level;
    return(0);
}                        /* end of controldata */
...
/* Process the value assigned to the matched parameter */
    ...
    else if (match(Tok[0],w_QUALITY));   /* FIXME: Qstep = t; */   /* Quality time step   */
...
static int
getpumpcurve(float first, char **tokens, int ntokens)
/*
**--------------------------------------------------------
...
*/
{
    ...
    for (j = 4; j < ntokens || n < 5; j++, n++)
    {
        if (!getfloat (tokens[j], &point[n])) return 202;
    }

    if (n == 1)                /* Const. HP curve       */
    {
        if (point[0] <= 0.0) return 202;
        Pump[Npumps].Ptype = CONST_HP;
        Link[Nlinks].Km = point[0];
    }
    else
    {
